...
public class AuthenticationScreen extends BaseScreen{
    ...
    public AuthenticationScreen fillEmail(String email){
        waitElement(editTextEmail, 15);
        type(editTextEmail, email);
        return this;
    }

    public AuthenticationScreen fillPassword(String password){
        waitElement(editTextPassword, 30);
...
//        return this;
    ...
    public AuthenticationScreen isErrorMessageContainsTextInAlertUser(String text){
        Alert alert = new WebDriverWait(driver, 5)
                .until(ExpectedConditions.alertIsPresent());
        driver.switchTo().alert();
        Assert.assertTrue(alert.getText().contains(text));
    ...
    public AuthenticationScreen isErrorMessageContainsTextInAlertWrongLogin(String text){
        Alert alert = new WebDriverWait(driver, 5)
                .until(ExpectedConditions.alertIsPresent());
        driver.switchTo().alert();
        Assert.assertTrue(alert.getText().contains(text));
    ...
    public AuthenticationScreen isErrorMessageContainsTextInAlertWrongPassword(String text){
        Alert alert = new WebDriverWait(driver, 5)
                .until(ExpectedConditions.alertIsPresent());
        driver.switchTo().alert();
        Assert.assertTrue(alert.getText().contains(text));
